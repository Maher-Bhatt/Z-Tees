...
const pillars = [
  {
    icon: "🎬",
    title: "Reels",
    desc: "styling videos, drop reveals, quick fits",
    color: "#FF6B00",
  },
  {
    icon: "📦",
    title: "Behind the Scenes",
    desc: "packing orders, making the tees",
    color: "#FF2D78",
  },
  {
    icon: "⭐",
    title: "Customer Fits",
    desc: "real people, real fits, no filter",
    color: "#FFE600",
    textColor: "#0D0D0D",
  },
];
...
export default function InstagramSection() {
  ...
  return (
    <section
      ref={sectionRef}
      className="bg-background py-20 px-4 sm:px-6 border-t-2 border-white/10"
    >
      <div className="max-w-6xl mx-auto">
        <div className="mb-10">
          ...
          <p className="reveal reveal-delay-1 font-body text-muted-foreground mt-3 text-sm max-w-md">
            we post Reels. we post drops. we post behind the scenes.
            follow us or miss out. simple.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10">
          {pillars.map((p, i) => (
            ...
            >
              <div
                className="font-display text-5xl mb-4 w-14 h-14 flex items-center justify-center"
                style={{ background: p.color }}
              >
                {p.icon}
              </div>
              <h3
                className="font-display text-2xl text-brand-white mb-2"
        ...
        <div className="reveal reveal-delay-4 border-2 border-white/10 p-6 mb-10 text-center">
          <p className="font-accent text-muted-foreground text-xs uppercase tracking-widest">
            {/* PLACEHOLDER: Replace this div with Behold or Elfsight Instagram embed widget when live */}
            [ instagram feed embed goes here — use Behold.so or Elfsight widget ]
          </p>
        </div>
